...
def check_haveibeenpwned(email):
    ...
    url = f"https://haveibeenpwned.com/api/v2/breachedaccount/{str(email)}"
...
def grab_password(email):
    ...
    return False if "Error" in req.text or len(result)==2 else result[1:-1]

def parse_data(email,np):
    if data := check_haveibeenpwned(email):
        status(f"Haveibeenpwned website results: {Y}{len(data)}")
        ...
        for website in data:
            ...
            status(B+line)
        if not np:
            if p := grab_password(email):
                status("Plaintext password(s) found!")
                for pp in p:
                    print(f"{C} │{B}  └──── {W}" + pp.split(":")[1])
            else:
